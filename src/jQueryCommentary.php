...
function jQueryCommentary_getmoduleinfo(): array
{
    return [
        ...
        'version' => '1.3.0',
        'category' => 'Gameplay',
        'download' => 'https://github.com/stephenKise/lotgd-modules',
...
function formatComment(array $row): array
{
    ...
    if ($row['comment'] != sanitizeHTML($row['comment'])) {
        $row['comment'] = sanitizeHTML($row['comment']);
    }
    $edit = '';
    ...
    $row['comment'] = sanitizeHTML(stripslashes($row['comment']));
    return  modulehook(
        'chat-format',
        [
            'author' => $row['author'],
            ...
            'formattedComment' => appoencode("<div class='jQuery-message'>
                    `0$edit `)(`2$timeStamp`)) $bioLink {$row['comment']}
                </div>", true)
        ]
    );
}
